...
class VllmWrapperWg: # Thi is a developing class for eval and test
	def __init__(self, config, tokenizer):
		...
		# print("LLM initialized")  # Comment out initialization print
...
class LLMAgentProxy:
	...
	def rollout(self, dataproto: DataProto, val=False, global_step: int = 0, total_training_steps: int = 1):
		...
		# Get seed information from dataproto, ensure it's passed to environment reset
		seed = dataproto.meta_info.get('seed', None)
		
		# Reset environment, pass seed, and pass global step information during training
		if not val:
			env_outputs = es_manager.reset(seed=seed, global_step=global_step, total_training_steps=total_training_steps)
		else:
