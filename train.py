...
if not ray.is_initialized():
    ...
    ray.init(address='local', runtime_env={
        'env_vars': {
...
def add_dependency_and_validate_config(config):
    ...
    # Check if model is Qwen by examining the model path or config
    def is_qwen_model(model_path):
        # Check if path contains qwen
        if "qwen" in model_path.lower():
            return True
        # Check if it's a local checkpoint with Qwen2 architecture
        import os
    ...
    assert is_qwen_model(config.model_path) or (not config.enable_response_mask), \
    ...
    # Calculate effective filter ratio based on filter mode
    if hasattr(config.actor_rollout_ref.rollout, 'filter_mode') and config.actor_rollout_ref.rollout.filter_mode == 'two_stage':
        # Two-stage filtering: effective ratio = variance_filter_ratio * score_filter_ratio
        effective_filter_ratio = config.actor_rollout_ref.rollout.variance_filter_ratio * config.actor_rollout_ref.rollout.score_filter_ratio
    else:
        # Original filtering
        effective_filter_ratio = config.actor_rollout_ref.rollout.rollout_filter_ratio
    
    assert config.es_manager.train.env_groups * config.es_manager.train.group_size * effective_filter_ratio >= config.ppo_mini_batch_size, \
        f"env_groups * group_size * effective_filter_ratio ({config.es_manager.train.env_groups * config.es_manager.train.group_size * effective_filter_ratio}) must be greater than or equal to ppo_mini_batch_size ({config.ppo_mini_batch_size}). Note that effective rollouts for update is env_groups * group_size * effective_filter_ratio."
